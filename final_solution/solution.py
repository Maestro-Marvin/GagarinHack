...
from final_solution.absa.model import (
...
def score_texts(
    ...
    preprocessor = ner.NER(
        f"{path_to_solution}/ner/new_names_and_synonyms.csv"
    )
    ...
    model = TransformerClassificationModel(
        rubert_tiny, num_classes=5, num_dense_layers=2
    ).to(device)
    tokenizer_tiny = AutoTokenizer.from_pretrained(rubert_tiny)
    model.load_state_dict(
        torch.load(
            f"{path_to_solution}/absa/rubert_tiny_2fc",
            map_location=torch.device(device),
        )
    )
    return [
        score_text(
            [message], preprocessor, model, tokenizer_tiny, device
        )
